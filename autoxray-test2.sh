...
tmp_limits="$(mktemp)"
curl --connect-timeout 10 --max-time 30 -fsSL "https://raw.githubusercontent.com/kartazon/autoxray-setup/main/limits.conf" -o "$tmp_limits"
...
curl --connect-timeout 10 --max-time 30 -fsSL "https://raw.githubusercontent.com/kartazon/autoxray-setup/main/limits-xray.conf" -o "$tmp_xray_limits"
...
xray_uuid_rty_vision=$(xray uuid)
xray_uuid_rty_xhttp=$(xray uuid)
xray_uuid_tls_vision=$(xray uuid)
xray_uuid_tls_xhttp=$(xray uuid)
...
xray_shortid_rty_vision=$(openssl rand -hex 8)
xray_shortid_rty_xhttp=$(openssl rand -hex 8)
...
export \
  xray_uuid_rty_vision \
  xray_uuid_rty_xhttp \
  xray_uuid_tls_vision \
  xray_uuid_tls_xhttp \
  xray_privateKey_vrv \
  xray_publicKey_vrv \
  xray_shortid_rty_vision \
  xray_shortid_rty_xhttp \
  xray_sspasw_vrv \
  DOMAIN path_subpage path_xhttp WEB_PATH socksUser socksPasw
        ...
        "clients": [
          {
            ...
            "id": "${xray_uuid_rty_vision}"
          },
		  {
             "id": "${xray_uuid_rty_xhttp}"
          }		  
        ],
        "decryption": "none",
        "fallbacks": [
...
OUT_REALITY_VISION='{
  ...
  "settings": {
    "vnext": [{
      "address": "$DOMAIN",
      "port": 443,
      "users": [{ "id": "${xray_uuid_rty_vision}", "flow": "xtls-rprx-vision", "encryption": "none" }]
    }]
  },
  "streamSettings": {
    "network": "raw",
    ...
    "realitySettings": {
      ...
      "password": "${xray_publicKey_vrv}", "shortId": "${xray_shortid_rty_vision}", "spiderX": "/"
    }
  }
}'
...
OUT_REALITY_XHTTP='{
  ...
  "settings": {
    "vnext": [{
      "address": "$DOMAIN",
      "port": 443,
      "users": [{ "id": "${xray_uuid_rty_xhttp}", "encryption": "none" }]
    }]
  },
  "streamSettings": {
    "network": "xhttp",
    ...
    "realitySettings": {
      "show": false, "fingerprint": "chrome", "serverName": "$DOMAIN",
      "password": "${xray_publicKey_vrv}", "shortId": "${xray_shortid_rty_xhttp}", "spiderX": "/"
    }
  }
}'
...
OUT_VISION='{
  ...
  "settings": {
    "vnext": [{
      "address": "$DOMAIN",
      "port": 8443,
      "users": [{ "id": "${xray_uuid_tls_vision}", "flow": "xtls-rprx-vision", "encryption": "none" }]
    }]
  },
  "streamSettings": {
    "network": "raw",
...
OUT_XHTTP='{
  ...
  "settings": {
    "vnext": [{
      "address": "$DOMAIN",
      "port": 8443,
      "users": [{ "id": "${xray_uuid_tls_xhttp}", "encryption": "none" }]
    }]
  },
  "streamSettings": {
    "network": "xhttp",
...
# Формирование ссылок
linkRTY1="vless://${xray_uuid_rty_vision}@$DOMAIN:443?security=reality&type=tcp&headerType=&path=&host=&flow=xtls-rprx-vision&sni=$DOMAIN&fp=chrome&pbk=${xray_publicKey_vrv}&sid=${xray_shortid_rty_vision}&spx=%2F#vlessRAWrealityVISION-autoXRAY"

linkRTY2="vless://${xray_uuid_rty_xhttp}@$DOMAIN:443?security=reality&type=xhttp&headerType=&path=%2F$path_xhttp&host=&mode=stream-one&extra=%7B%22xmux%22%3A%7B%22cMaxReuseTimes%22%3A%221000-3000%22%2C%22maxConcurrency%22%3A%223-5%22%2C%22maxConnections%22%3A0%2C%22hKeepAlivePeriod%22%3A0%2C%22hMaxRequestTimes%22%3A%22400-700%22%2C%22hMaxReusableSecs%22%3A%221200-1800%22%7D%2C%22headers%22%3A%7B%7D%2C%22noGRPCHeader%22%3Afalse%2C%22xPaddingBytes%22%3A%22100-1000%22%2C%22scMaxEachPostBytes%22%3A1500000%2C%22scMinPostsIntervalMs%22%3A20%2C%22scStreamUpServerSecs%22%3A%2260-240%22%7D&sni=$DOMAIN&fp=chrome&pbk=${xray_publicKey_vrv}&sid=${xray_shortid_rty_xhttp}&spx=%2F#vlessXHTTPrealityEXTRA-autoXRAY"

linkTLS1="vless://${xray_uuid_tls_vision}@$DOMAIN:8443?security=tls&type=tcp&headerType=&path=&host=&flow=xtls-rprx-vision&sni=$DOMAIN&fp=chrome&spx=%2F#vlessRAWtlsVision-autoXRAY"

linkTLS2="vless://${xray_uuid_tls_xhttp}@$DOMAIN:8443?security=tls&type=xhttp&headerType=&path=%2F${path_xhttp}&host=&mode=auto&extra=%7B%22xmux%22%3A%7B%22cMaxReuseTimes%22%3A%221000-3000%22%2C%22maxConcurrency%22%3A%223-5%22%2C%22maxConnections%22%3A0%2C%22hKeepAlivePeriod%22%3A0%2C%22hMaxRequestTimes%22%3A%22400-700%22%2C%22hMaxReusableSecs%22%3A%221200-1800%22%7D%2C%22headers%22%3A%7B%7D%2C%22noGRPCHeader%22%3Afalse%2C%22xPaddingBytes%22%3A%22100-1000%22%2C%22scMaxEachPostBytes%22%3A1500000%2C%22scMinPostsIntervalMs%22%3A20%2C%22scStreamUpServerSecs%22%3A%2260-240%22%7D&sni=$DOMAIN&fp=chrome&spx=%2F#vlessXHTTPtls-autoXRAY"
...
Открыт локальный socks5 на порту 10808, и http на 10809.
